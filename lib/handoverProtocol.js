/**
 * Local handover / return protocol state (no API yet).
 */

/**
 * @typedef {object} HandoverProtocolPhoto
 * @property {string} id
 * @property {string} uri Placeholder URI until image picker is wired
...
/** @type {Map<string, HandoverProtocol>} */
const protocolsByBookingId = new Map();
...
export function handoverProtocolSignatureHasContent(signature) {
  if (!signature || !Array.isArray(signature.strokes)) return false;
...
/**
 * @param {string} bookingId
 * @returns {HandoverProtocol | null}
 */
export function getHandoverProtocol(bookingId) {
  const key = String(bookingId);
  const stored = protocolsByBookingId.get(key);
  return stored ? cloneHandoverProtocol(stored) : null;
}

/**
 * @param {HandoverProtocol} protocol
 */
export function saveHandoverProtocol(protocol) {
  protocolsByBookingId.set(String(protocol.bookingId), cloneHandoverProtocol(protocol));
}
 ...
 * @returns {HandoverProtocol}
 */
export function getOrCreateHandoverProtocol(bookingId) {
  const existing = getHandoverProtocol(bookingId);
  if (existing) return existing;
  return createEmptyHandoverProtocol(bookingId);
}

/**
 * @param {HandoverProtocolSection | null | undefined} section
 * @returns {boolean}
 */
/**
...
export function __clearHandoverProtocolStoreForTests() {
  protocolsByBookingId.clear();
}

/**
 * Mock photo picker — replace with expo-image-picker when available.
 * @returns {Promise<HandoverProtocolPhoto | null>}
 */
export function pickMockHandoverProtocolPhoto() {
  const id = `mock-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  return Promise.resolve({
    id,
    uri: `mock://handover-photo/${id}`,
    addedAt: Date.now(),
  });
}
